...
- **MUST** preserve the entire question object verbatim across read/write cycles. No field dropping, no reordering of nested objects, no `globalId` rewriting.
...
A consumer that imports an LC-JSON document, modifies it, and re-exports MUST preserve all reserved-type questions verbatim in the exported document — including their `globalId`, `type`, `points`, `prompt`, and any additional fields that were present on import.
