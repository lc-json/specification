...
Terms are organised in four groups: **core concepts**, **artifact and hierarchy**, **conformance**, and **identity, versioning, and extensions**.
...
A top-level document type defined by the specification. LC-JSON 1.0 defines two artifact types: **Course** (hierarchical) and **QuestionSet** (flat). The artifact a document represents is signalled by its `documentType` root field.
...
A `type` discriminator value **not** listed in the spec's discriminator enum at all — typically a tool-specific extension under a `type` name that the producer's tooling recognises but other tools do not. Unknown types follow the same round-trip preservation contract as reserved types (§6).
